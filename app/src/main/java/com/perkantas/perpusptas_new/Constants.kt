package com.perkantas.perpusptas_new

object Constants {
    //endpoints
    const val BASE_URL = "http://10.104.50.107:8000"
    const val LOGIN_URL = "/api/login"
    const val REGISTER_URL = "/api/register"
    const val PROFILE_URL = "/api/myProfile"
    const val CATEGORY_URL = "/api/categories"
    const val INDEX_URL = "/api/index"
    const val PROFILE_CHECK = "/api/checkProfile"
    const val RENT_URL = "/api/rent"
    const val NOTIFICATION_URL = "/api/notifications"
    const val NOTIFICATION_URL_READ = "/api/notifications/{id}"
    const val PASSWORD_CHANGE_URL = "/api/change-password-dummy"
    const val PASSWORD_RESET = "/api/forgot-password"
}